...
const HeroSection = () => {
  ...
  }[heroSettings.titleSize] || 'text-4xl sm:text-5xl lg:text-6xl';

  const subtitleSizeClass = {
    'sm': 'text-lg',
    'md': 'text-xl',
    'lg': 'text-2xl'
  }[heroSettings.subtitleSize] || 'text-xl';

  const textColorClass = {
    'foreground': 'text-black',
    'primary': 'text-primary',
    'muted-foreground': 'text-muted-foreground'
  }[heroSettings.textColor] || 'text-black';

  const paddingClass = {
    'compact': 'py-12 lg:py-16',
    'comfortable': 'py-16 lg:py-24',
    'roomy': 'py-20 lg:py-32'
  }[heroSettings.padding] || 'py-16 lg:py-24';

  return (
    <section className="relative overflow-hidden bg-white" style={backgroundStyle}>
      <div className={cn("max-w-7xl mx-auto px-4 sm:px-6 lg:px-8", paddingClass)}>
        <div className={textPositionClass}>
          <motion.h1
            className={cn(titleSizeClass, "font-bold mb-4 leading-tight", textColorClass)}
          ...
          {heroSettings.subtitle && (
            <motion.p
              className={cn(subtitleSizeClass, "text-gray-600 mb-8")}
          ...
          <motion.div
            className={cn("flex flex-col sm:flex-row gap-4", justifyClass)}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
